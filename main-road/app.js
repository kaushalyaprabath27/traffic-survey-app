...
function init() {
    ...
    // Generate Location Numbers (E1-E3)
    for (let i = 1; i <= 3; i++) {
  ...
  setTimeout(() => {
      try {
          ...
          if (cfgStr) {
              ...
              const inName = cfg.mrIn || 'In';
              const outName = cfg.mrOut || 'Out';
              
              // Update panel headers
              const inPanelH2 = document.querySelector('.in-panel .panel-header h2');
              if (inPanelH2) inPanelH2.innerHTML = inName + ' <i class="fa-solid fa-arrow-down-long"></i>';
              
              const outPanelH2 = document.querySelector('.out-panel .panel-header h2');
              if (outPanelH2) outPanelH2.innerHTML = outName + ' <i class="fa-solid fa-arrow-up-long"></i>';
              
              // Update all buttons data-dir attributes
              document.querySelectorAll('.in-panel .vehicle-btn').forEach(btn => btn.setAttribute('data-dir', inName));
              document.querySelectorAll('.out-panel .vehicle-btn').forEach(btn => btn.setAttribute('data-dir', outName));
          }
      } catch(e) {}
  }, 100);
