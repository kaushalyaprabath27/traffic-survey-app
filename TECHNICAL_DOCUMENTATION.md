...
- **Passwords were stored in plaintext.** `handleVerifyOTP` wrote the password submitted at signup directly into a registry Sheet cell with no hashing. This was a real weakness, not a documented trade-off. It has since been fixed and deployed: signup, login, and password reset now use salted SHA-256 (`Utilities.computeDigest`), and the one-time migration was run against the live registry on 28 August 2026, hashing 12 pre-existing plaintext rows in place with no plaintext value retained (execution log and details in `docs/idempotency_and_password_hashing.md`).
...
The password-hashing fix above, and a separate eventId-based deduplication fix for `submit_batch` (see `docs/idempotency_and_password_hashing.md`), have both been deployed and empirically confirmed against the live backend; neither was silently patched, both are disclosed here with their verification evidence. The OTP and Admin ID findings remain unfixed, disclosed platform design limits, not silently patched either. The tool's threat model assumes administrator spreadsheets contain only aggregate traffic counts with no personally identifying information, and is not intended for sensitive personal data.
