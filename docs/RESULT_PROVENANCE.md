...
|---|---|---|---|
| Main-road validation (Tables 5, 6, 7; 231 video-confirmed vehicles) | 14 Jul 2026 (field session) | No committed raw per-vehicle data exists for this session (Limitations, above) — the aggregate figures reported predate this repository's earliest committed validation artifacts and cannot be tied to a specific commit | Not backend-dependent — this is an offline video-vs-app timestamp comparison, not a live-backend measurement |
