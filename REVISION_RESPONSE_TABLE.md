...
| R2-8 | Table 3 restates the paragraph above it and overlaps Table 4 | **Applied (30 Aug 2026, author confirmed):** Table 3 cut, one row folded into prose, Tables 4-9 renumbered to 3-8 throughout | `SUBMISSION_NOTES.md` §9, whole manuscript |
...
| B1 | Admin ID weakness misattributed to platform | Widened to 12 chars (32-symbol alphabet); premise corrected (9,000 not 10,000 values); manuscript no longer attributes this to the platform. Round 3: self-narration ("that attribution was wrong") removed, paragraph restated as plain fact per supplied text; migration approved by author but not yet executable from here (no deployed write action; new IDs generated, awaiting manual registry edit) | Security model |
...
| B5 | Dead gesture handler, data minimisation, site-detail check | Gesture handler NOT removed (premise false); manuscript overclaim corrected, then self-narration around that correction also removed (round 3); a second, uncaught instance of the same overclaim found and fixed (Data integrity safeguards body, not just Table 2/Limitations); data minimization proposed then explicitly declined by the author; site-detail field checked, confirmed absent | Data integrity safeguards, Table 2, Limitations |
| C1 | Main-road irreproducibility, video deleted | Independent search done (repo, full git history, all branches, disk); found a candidate file outside the repository, reported; **author confirmed it is unrelated -- resolved** | `REVISION_DECISIONS_NEEDED.md` |
...
| Round 3, Blocker 4 | Ungrammatical highlight bullet (repeated "against") | Replaced verbatim per supplied wording | Highlight bullet 2 |
| Round 3, Blocker 1 | ADM-5505 assumed still live in Figure 2 | Checked fresh (read-only): not live, not in Figure 2 -- both already fixed in Part 0; reported rather than redone | Figure 2 |
